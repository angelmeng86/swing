...
- (void)setTooltipVisible:(BOOL)tooltipVisible animated:(BOOL)animated atTouchPoint:(CGPoint)touchPoint atChartView:(JBChartView*)chartView
{
    ...
    dispatch_block_t adjustTooltipPosition = ^{
		...
		if (chartView && !CGPointEqualToPoint(touchPoint, CGPointZero))
        {
            ...
            self.tooltipView.frame = CGRectMake(convertedTouchPoint.x - ceil(self.tooltipView.frame.size.width * 0.5), CGRectGetMinY(chartView.frame) - self.tooltipView.frame.size.height -  self.tooltipTipView.frame.size.height, self.tooltipView.frame.size.width, self.tooltipView.frame.size.height);
...
{
    [self setTooltipVisible:tooltipVisible animated:animated atTouchPoint:CGPointZero atChartView:chartView];
}
